  ...
  <div class="gate-table">
    ...
    <div class="form-wrapper">
      <div class="table-header-warpper">
        ...
        <div class="col2 col">出勤次数</div>
        <div class="col3 col">请假次数</div>
        <div class="col4 col">缺勤次数</div>
        <div class="col5 col">迟到次数</div>
      ...
      <div class="table-header-warpper list-item" v-for="(item, index) in pagingList[currentPage - 1]" :key="index">
        ...
        <div class="col2 col">{{item.att_number}}</div>
        <div class="col3 col">{{item.leave_number}}</div>
        <div class="col4 col">{{item.abs_number}}</div>
        <div class="col5 col">{{item.late_number}}</div>
...
<script>
  ...
  import { GATE_FORM } from '../utils/const.js'
    ...
    methods: {
      showForm() {
        ...
        this.formComponent = this.$createForm({
            $props: {
              inputList: GATE_FORM, // 之后改成const放在utils
              tableName: 'atendence'
            },
            $events: {
      ...
      flushData(text) {
        if (text === 'flush') { // mounted时使用，请求数据并分页
          getData('atendence').then(res => {
            let info = res.data // res.data.data才是
            this.attendMessages = info.data
            this.paging(this.attendMessages)
          })
        } else {
          this.hideForm()
          this.showToast(text)
          ...
          getData('atendence').then(res => {
            let info = res.data
            this.attendMessages = info.data
            this.paging(this.attendMessages) // 公有方法,各组件绑定this调用即可
          })
        }
      },
      deleteItem(item) {
        ...
        // 携带表名，主键名，主键值
        deleteData('atendence', 'emp_number', item.emp_number).then(res => {
